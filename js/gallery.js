...
function LandingPageScrollTrigger() {
    ...
    document.querySelector("#codeby a").addEventListener('pointerenter', function(){ // HoverAnimation---------------------
        gsap.to('#ImgWrapper', { backgroundColor: "#f0f0f0", })
        gsap.to('#codeby a', { color: "#2e2e2e", })
        gsap.to('#codeby a span', { color: "#000000", })
    });
    document.querySelector("#codeby a").addEventListener('pointerleave', function() {
